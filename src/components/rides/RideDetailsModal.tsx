...
function cancelledByLabel(
  role: NonNullable<RideRow["cancelledBy"]>["role"],
  t: (path: string) => string,
) {
  if (role === "driver") return t("rides.cancelledByDriver");
  if (role === "passenger") return t("rides.cancelledByPassenger");
  return t("rides.cancelledByOther");
}
...
function RatingBlock({
  ...
  return (
    <section className="rounded-xl border border-pika-border bg-pika-page/60 p-4">
      ...
      <div className="mt-3">
        <StarRating value={stars} iconClassName="h-5 w-5" emptyLabel={t("rides.noRating")} />
      </div>
      <p className="mt-3 text-sm leading-relaxed text-pika-ink">
        {comment ? (
          comment
        ) : (
          <span className="text-pika-muted italic">{t("rides.noComment")}</span>
        )}
      </p>
    </section>
  );
    ...
    >
      ...
      >
        ...
        <div className="mb-6 flex flex-wrap items-center gap-2">
          ...
          {ride.commissionLabel !== "—" ? (
            <span className="text-sm text-pika-muted">
              {t("rides.commission")} {ride.commissionLabel}
            </span>
          ) : null}
          {ride.distanceLabel ? (
        ...
        {ride.startTimeLabel || ride.endTimeLabel || ride.durationLabel ? (
          <div className="mb-6 flex flex-wrap items-center gap-x-4 gap-y-2 rounded-xl border border-pika-border bg-pika-page/60 px-4 py-3 text-sm">
            <span className="inline-flex items-center gap-2 text-pika-ink">
              ...
              <span>
                <span className="font-medium text-pika-muted">{t("rides.start")}: </span>
                {ride.startTimeLabel || "—"}
              </span>
            </span>
            <span className="text-pika-ink">
              <span className="font-medium text-pika-muted">{t("rides.end")}: </span>
              {ride.endTimeLabel || "—"}
            </span>
            {ride.durationLabel ? (
              <span className="font-semibold text-pika-ink">
                {t("rides.durationLabel")}: {ride.durationLabel}
              </span>
            ) : null}
          </div>
        ) : null}
          ...
          <div className="mb-6 flex items-start gap-3 rounded-xl border border-red-200 bg-red-50 p-4">
            ...
            <div className="min-w-0">
              <p className="text-sm font-semibold text-red-700">
                {cancelledByLabel(ride.cancelledBy.role, t)}
              </p>
        ...
        <div className="grid grid-cols-1 gap-4 sm:grid-cols-2">
          <DetailField label={t("common.passenger")}>
            <span className="font-semibold">{ride.passenger}</span>
          </DetailField>
          <DetailField label={t("common.driver")}>
            <span className="font-semibold">{ride.driver}</span>
          </DetailField>
        </div>

        <DetailField label={t("rides.route")} className="mt-4">
        ...
        <DetailField label={t("rides.vehicle")} className="mt-4">
          <div className="flex flex-col gap-2">
            <span className="inline-flex items-start gap-2">
              <FontAwesomeIcon
                icon={faCar}
              ...
              />
              <span>
                <span className="font-medium text-pika-muted">{t("rides.brandModel")}: </span>
            ...
            <span className="inline-flex items-start gap-2">
              ...
              />
              <span>
                <span className="font-medium text-pika-muted">{t("rides.plate")}: </span>
                {ride.vehiclePlate}
              </span>
            </span>
            <span className="inline-flex items-start gap-2">
              ...
              />
              <span>
                <span className="font-medium text-pika-muted">{t("rides.color")}: </span>
                {ride.vehicleColor}
              </span>
            </span>
          </div>
        ...
        <div className="mt-6 space-y-4">
          ...
          />
          <RatingBlock
            title={t("rides.driverRating")}
            subtitle={t("rides.driverRatingHint")}
